...
if status == 'request':
	...
	for directory in tmp_dirs:
		...
		for root, dirs, files in os.walk(full_path, topdown=False):
		    for name in files:
		    	...
		    	if request_file_key not in full_name:			#if full name is not related to request
		        	os.remove(full_name)
	...
	auth_status = control.check_auth_status(user_request_id)					#check if request requires authentication
